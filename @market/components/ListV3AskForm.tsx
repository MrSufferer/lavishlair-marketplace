...
export function ListV3AskForm({
  ...
  const { AsksV12 } = useContractContext()
  ...
  const handleOnSubmit = useCallback(
    async (
      ...
      try {
        ...
        const promise: Promise<ContractTransaction> = isUpdate
          ? AsksV12.setAskPrice(tokenAddress, tokenId, values.amount, values.currency.id)
          : AsksV12.createAsk(
              tokenAddress,
              tokenId,
              values.amount,
    ...
    [AsksV12, address, handleTx, isUpdate, onConfirmation, tokenAddress, tokenId]
  )
